<?php
add_action( 'save_post', 'update_news_slideshow' );

function update_news_slideshow( $post_id ) {
	$post_categories = wp_get_post_categories( $post_id );
	if ( in_array(1, $post_categories ) ) {
		$slideshow_html ="";
		$slide_count = 1;
		...
		foreach( $news_posts as $post ) :
			if( has_post_thumbnail( $post->ID ) ) {
				$slideshow_html .= '<div rel="'.$slide_count++.'" class="slide"><h4><a href="http://blog.cuttingedgestencils.com/'.$post->post_name.'.html">'.$post->post_title.'</a></h4><a href="http://blog.cuttingedgestencils.com/'.$post->post_name.'.html">'.get_the_post_thumbnail($post->ID, array(200,200)).'</a></div>';
			}
		endforeach;
		$Tes = file_put_contents(dirname(dirname(dirname(__FILE__)))."/slideshow.php" ,$slideshow_html, LOCK_EX);
	}
}
add_action( 'edit_category_form_fields', 'related_categories' );
add_action( 'edit_category', 'save_related_categories');
...
<tr class="form-field">
	...
	<li class="popular-category"><label class="selectit"><input type="checkbox" name="post_category[]" value="0"<?php echo $checked; ?>>All categories</label></li>	
<?php
$related_categories_arr = explode(",", $related_categories);
...
foreach($categories_options as $categories_option) {
	if(strlen($categories_option) < 5) {continue;}
...
function save_related_categories($category){
	$category = $_POST[tag_ID];
	$related_category_ids = implode(",",$_POST[post_category]);
	update_option( "related_categories_".$category, $related_category_ids );
}
