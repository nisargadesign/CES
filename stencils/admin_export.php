...
*/
	...
	if ($table == "items" || $table == "items_files") {
		...
		if (strlen($id)) {
			...
			if (($sql_where == " WHERE o.order_id=oi.order_id ") && ($type == "filtered")) {
				$db->query("SELECT status_id FROM " . $table_prefix . "order_statuses WHERE (is_list=1 OR is_list IS NULL)");
				if ($db->next_record()) {
					$orders_statuses = array();
					do {
						$orders_statuses[] = $db->f("status_id");						
					} while ($db->next_record());					
					$sql_where .= " AND o.order_status IN ( " . $db->tosql($orders_statuses, INTEGERS_LIST) . " ) ";
				}			
			}
		}
