...
*/
	...
	$sql .= " i.price, i.sales_price, i.is_sales, i.properties_price, i.tax_id, i.tax_free, i.stock_level ";
	$sql .= " FROM (" . $table_prefix . "items i ";
	...
	while ($db->next_record()) {
		...
		$item_name 		= htmlentities($item_name, ENT_QUOTES);	
		$item_name    = rtrim(trim(str_replace($order, $replace, $item_name)));
		...
		write_to("<" . $schema_type . ":product_type>" . str_replace("&", " ", $google_base_type_name) . "</" . $schema_type . ":product_type>" . $eol);
		...
		if (!check_desc_for_googlebase($description)) {
			$description = rtrim(trim(str_replace($order, $replace, (get_translation($db->f("short_description"))))));
		}
		if (!check_desc_for_googlebase($description)) {
			$description = rtrim(trim(str_replace($order, $replace, (get_translation($db->f("features"))))));
		}
		if (!check_desc_for_googlebase($description)) {
			$description = rtrim(trim(str_replace($order, $replace, (get_translation($db->f("item_name"))))));
		}
		if (!check_desc_for_googlebase($description)) {
			continue;
		}

		//$description = preg_replace(array('/&/', '/"/'), array('&amp;', '&quot;'), $description);
	...
	function check_desc_for_googlebase($description) {
		return (strlen($description) && strlen(trim(strip_tags($description))));
	}
?>
