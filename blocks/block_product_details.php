...
// ################ BEGIN: PREV NEXT LINKS ################
	...
	if($item_id_key !== false && isset($products_ids[$item_id_key+1])){
		...
		$sql .= " WHERE i.item_id = " . $db->tosql($products_ids[$item_id_key+1], INTEGER);
		$db->query($sql);
		if ($db->next_record())
		{
			$next_item_id = $db->f("item_id");
...
// ################ END: PREV NEXT LINKS ################
/*$sql  = " SELECT item_order, item_id, item_name " ;
...
if ($db->next_record())
	{
	$t->set_var("ORDER", "ORDER:" .$db->f("item_order") . "<br>");
	$t->set_var("ID", "ID:" .$db->f("item_id") . "<br>");
	$t->set_var("NAME", "NAME:" .$db->f("item_name") . "<br>");
	
	$prev_order = $db->f("item_order") - 1;
	$next_order = $db->f("item_order") + 1;
	}
	

$sql2  = " SELECT item_order, item_id, item_name " ;
$sql2 .= " FROM (" . $table_prefix . "items )";
$sql2 .= " WHERE item_order = " . $prev_order ;

$db->query($sql2);

if ($db->next_record())
	{
	$t->set_var("PREV_ORDER", "ORDER:" .$db->f("item_order") . "<br>");
	$t->set_var("PREV_ID", "ID:" .$db->f("item_id") . "<br>");
	$t->set_var("PREV_NAME", "NAME:" .$db->f("item_name") . "<br>");
	}

$sql3  = " SELECT item_order, item_id, item_name " ;
$sql3 .= " FROM (" . $table_prefix . "items )";
$sql3 .= " WHERE item_order = " . $next_order ;

$db->query($sql3);

if ($db->next_record())
	{
	$t->set_var("NEXT_ORDER", "ORDER:" .$db->f("item_order") . "<br>");
	$t->set_var("NEXT_ID", "ID:" .$db->f("item_id") . "<br>");
	$t->set_var("NEXT_NAME", "NAME:" .$db->f("item_name") . "<br>");
	}*/
