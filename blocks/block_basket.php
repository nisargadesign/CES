<?php
  ...
  $t->set_file("block_body", $html_template);
	...
	$cart_image = get_setting_value($vars, "image_type", 0);
	...
	if (!$rp_page) { $rp_page = get_custom_friendly_url("products.php"); }
	...
	if (is_array($shopping_cart))
	{
		...
		foreach ($shopping_cart as $cart_id => $item) {
			...
			$sql  = " SELECT i.item_name, i.friendly_url, i.item_type_id, i.item_code, i.manufacturer_code, i.short_description, ";
...
// --------- OPTION DISPLAY																											
									...
									if (preg_match("/^http\:\/\//", $property_value)) {
										$property_value = "<a href=\"".htmlspecialchars($property_value)."\" target=\"_blank\">" . basename($property_value) . "</a>";
									}
					...
					foreach ($item_coupons as $coupon_id => $coupon_info)
					{
						...
						if ($db->next_record()) {
							...
							if ($discount_number != $quantity) {
								...
								$properties_values .= "<br><a href=\"" . htmlspecialchars($remove_coupon_link) . "\" onclick=\"return confirmDelete('".COUPON_MSG."');\">" . COUPON_REMOVE_MSG . "</a>";
							}
						}
					}
				}
	...
	set_options($countries, $selected_country, "country_id");
		...
		$sql .= " ps.fast_checkout_image, ps.fast_checkout_width, ps.fast_checkout_height, ps.fast_checkout_alt ";
		...
		if (sizeof($fast_checkouts) > 0) {
			...
			$fast_checkout_country_show = get_setting_value($vars, "fast_checkout_country_show", 0);
			$fast_checkout_country_required = get_setting_value($vars, "fast_checkout_country_required", 0);
			$fast_checkout_state_show = get_setting_value($vars, "fast_checkout_state_show", 0);
			$fast_checkout_state_required = get_setting_value($vars, "fast_checkout_state_required", 0);
