<?php                           
	...
	$t->set_var("items_cols_thumb", "");
	$t->set_var("items_rows", "");
	$t->set_var("items_rows_thumb", "");
...
//---------------TESTIMONIALS CODE
	...
	if ($category_id == 39)
		{
		$html_template = get_setting_value($block, "html_template", "block_articles_list_TESTIMONIALS_THUMB.html");
		}
		elseif (($category_id == 40) || ($category_id == 41))
			{
			$html_template = get_setting_value($block, "html_template", "block_articles_list_FAQ.html");
			}
			else
				{
...
if ($current_category_id == 38)
	{
	$hideDaThing = "hideDaThing";
	}
	else
		{
		$hideDaThing = "blue";
		...
		$t->set_var("hideDaThing", $hideDaThing);

//echo "X: " . $current_category_id;
			...
			do
			{
				...
				if (strpos($list_fields, ",image_small,") && strlen($image_small)) {
					...
					if (!strlen($image_small_alt)) { $image_small_alt = $article_title; }
						...
						if(!$is_OG_image_set) {$meta_OG_image = $image_small; $is_OG_image_set = true;}	//Customization by Vital - Open Graph image
...
//-------------- TRICKY SLIDESHOW DIV THING
						
		if (($item_number == 9) || ($item_number == 18) || ($item_number == 27) || ($item_number == 36) || ($item_number == 45) || ($item_number == 54) || ($item_number == 63) || ($item_number == 72) || ($item_number == 81) || ($item_number == 90) || ($item_number == 99)) 
			{				
			$t->set_var("daDiv", "</div><div>");
			}
			else
				{				
				$t->set_var("daDiv", "");
					...
					$t->global_parse("image_small_block_thumb", false, false, true);
				} else {
					$t->set_var("image_small_block", "");
					$t->set_var("image_small_block_thumb", "");
				}
				...
				if (strpos($list_fields, ",image_large,") && strlen($image_large)) {
					...
					if (!strlen($image_large_alt)) { $image_large_alt = $article_title; }
						...
						if(!$is_OG_image_set) {$meta_OG_image = $image_large; $is_OG_image_set = true;}	//Customization by Vital - Open Graph image
					if (is_array($image_size)) {
						$t->set_var("width", "width=\"" . $image_size[0] . "\"");
						$t->set_var("height", "height=\"" . $image_size[1] . "\"");
					} else {
