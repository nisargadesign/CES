...
if (strlen(get_session("session_user_id"))) {
	//remove duplicate items ignoring size and quantity
	$sql = "DELETE from " . $table_prefix . "saved_items USING " . $table_prefix . "saved_items, " . $table_prefix . "saved_items as vtable WHERE (" . $table_prefix . "saved_items.cart_item_id>vtable.cart_item_id) AND (" . $table_prefix . "saved_items.item_id=vtable.item_id AND vtable.user_id=". $db->tosql(get_session("session_user_id"), INTEGER) .")";
	//$db->query($sql);
	//consolidate "loged-in" and cookie wishlists
	$wishlist_user_id = 0;
	if (isset($_COOKIE['wishlist_user_id']) && is_numeric($_COOKIE['wishlist_user_id'])) {
		$wishlist_user_id = $_COOKIE['wishlist_user_id'];
		$sql = "UPDATE " . $table_prefix . "saved_items SET user_id=". $db->tosql(get_session("session_user_id"), INTEGER) ." WHERE user_id=". $db->tosql($wishlist_user_id, INTEGER)." AND item_id NOT IN (SELECT item_id FROM (SELECT DISTINCT item_id FROM va_saved_items WHERE user_id = ".$db->tosql(get_session("session_user_id"), INTEGER) .") AS tmptable)";
		$db->query($sql);
		//get and load items to cookie wishlist
		$sql =  "SELECT DISTINCT item_id, item_name, quantity, price, date_added FROM " . $table_prefix . "saved_items WHERE user_id=" . $db->tosql(get_session("session_user_id"), INTEGER)." AND item_id NOT IN (SELECT item_id FROM (SELECT DISTINCT item_id FROM va_saved_items WHERE user_id = ".$db->tosql($wishlist_user_id, INTEGER) .") AS tmptable)";
		$db->query($sql);
		if ($db->next_record()) {
			$wishlist_item_values = "(NULL, 1, '".$db->f("item_id")."', 0, '".$db->tosql($_COOKIE['wishlist_user_id'], INTEGER)."', '1', '".$db->f("item_name")."', '".$db->f("quantity")."', 0, '".$db->f("price")."', TIMESTAMP('".$db->f("date_added")."'))";
	...
	$t->set_var("cart_retrieve_href", get_custom_friendly_url("cart_retrieve.php"));
	$t->set_var("user_home_href", get_custom_friendly_url("user_home.php"));
	$t->set_var("wishlist_message", "Your wishlist is saved on this browser on this computer only. If you want to see your wishlist from another computer please <a href=\"http://www.cuttingedgestencils.com/user_login.php?return_page=http%3A%2F%2Fwww.cuttingedgestencils.com%2Fuser_wishlist.php\">login to save it</a>.<br /><br />");
	...
	else
	{
		...
		$t->set_var("navigator", "");
		$t->parse("no_records", false);
		$t->set_var("wishlist_message", "Your wishlist is saved on this browser on this computer only. If you want to see your wishlist saved in our database please <a href=\"http://www.cuttingedgestencils.com/user_login.php?return_page=http%3A%2F%2Fwww.cuttingedgestencils.com%2Fuser_wishlist.php\">login here</a>.<br /><br />");
	}
