...
//function articles_details($block_name, $article_id, $category_id, $details_fields, $details_template)
  ...
  $t->set_file("block_body", $html_template);
	...
	if ($db->next_record()) {
		...
		if (!strlen($meta_title)) { $meta_title = $article_title; }
...
$t->set_var("PREPREV", "<div class=\"navigatorDetail\">");	
$t->set_var("MIDDLE", "&nbsp;&nbsp;.&nbsp;&nbsp;");	
$t->set_var("POSNEXT", "</div>");	

$t->set_var("PREV", "");	
$t->set_var("NEXT", "");	

//CURRENT				
$sql  = " SELECT a.article_id, a.category_id, c.category_name, a.article_order " ;
			$sql .= " FROM  (" . $table_prefix . "articles_assigned a ";
			$sql .= " LEFT JOIN " . $table_prefix . "articles_categories c ON c.category_id=a.category_id)";
			$sql .= " WHERE article_id > $article_id - 1 ORDER BY article_id LIMIT 1";	
			$db->query($sql);
		
			while ($db->next_record()) {
			//echo $db->f("category_id") . "<br>";
			$dacat = $db->f("category_id");
			$t->set_var("CURR", "CURRENT ARTICLE: " . $db->f("article_id") . " ORDER: " . $db->f("article_order") . "<br>");	
			$prev_order = $db->f("article_order") - 1;
			$next_order = $db->f("article_order") + 1;
			}
			//echo $prev_order . "<br>";
			//echo $next_order . "<br>";
...
$sql  = " SELECT a.article_id, a.category_id, c.category_name, a.article_order " ;
			$sql .= " FROM  (" . $table_prefix . "articles_assigned a ";
			$sql .= " LEFT JOIN " . $table_prefix . "articles_categories c ON c.category_id=a.category_id)";
			$sql .= " WHERE article_order = $prev_order";	
			$db->query($sql);
		
			while ($db->next_record()) {
				if ($db->f("category_id") == 5)	{
					if ($prev_order != 0)	{			
						$t->set_var("PREV", "<a href=\"article.php?category_id=5&article_id=" . $db->f("article_id") . "\" class=\"navigatorLink\">< PREVIOUS</a>");	
						}
				}
			}			
...
$sql  = " SELECT a.article_id, a.category_id, c.category_name, a.article_order " ;
			$sql .= " FROM  (" . $table_prefix . "articles_assigned a ";
			$sql .= " LEFT JOIN " . $table_prefix . "articles_categories c ON c.category_id=a.category_id)";
			$sql .= " WHERE article_order = $next_order";	
			$db->query($sql);
		
			while ($db->next_record()) {
				if ($db->f("category_id") == 5)	{
					$t->set_var("NEXT", "<a href=\"article.php?category_id=5&article_id=" . $db->f("article_id") . "\" class=\"navigatorLink\">NEXT ></a>");	
					}		
			}
